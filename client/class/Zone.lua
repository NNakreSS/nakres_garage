...
Zone.create = function(type, garage, index)
    ...
    if type ~= "parkOut" then
        ...
        garageZones[type .. "_" .. index].boxZone = BoxZone:Create(coords, size, size, {
            minZ = minz,
            maxZ = maxz,
            name = type,
...
Zone.createTargetPed = function(index, garage)
    if garage.type ~= 'house' then
        garageNpcs[index] = {
            ...
            job = (garage.type == 'job' or garage.type == 'gang') and {
                job = garage.job,
                jobType = garage.jobType
            }
        };
    end
end
    ...
    for index, value in pairs(garageNpcs) do
        ...
        if dst < 150 then
            if not value.created then
                ...
                local npc = CreatePed(0, value.ped, coord, value.heading or 0.0, false, true);
